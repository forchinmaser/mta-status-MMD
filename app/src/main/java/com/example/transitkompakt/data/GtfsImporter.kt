 ...
 *
 * Both share one downloaded zip ([zipFile]). What happens to that zip after
 * use depends on its size: at or under [MAX_KEPT_ZIP_BYTES] it's left on disk
 * indefinitely, so a later re-parse (TransitRepository's caches are
 * short-lived — see its CACHE_TTL_MS) costs no network. Over that size — a
 * big borough's stop_times.txt can be "hundreds of megabytes uncompressed" —
 * it's deleted right after use, and the next request for that feed
 * re-downloads it from scratch. That trades bandwidth for bounded storage on
 * the few feeds large enough to matter.
 */
